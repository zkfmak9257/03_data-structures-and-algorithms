package com.work.section01.timecomplexity;
 ...
 * - 특정 알고리즘이 대규모 데이터 셋에서 얼마나 잘 동작할지 예측하는데 중요0
 * */
public class Application1 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("선형시간 O(n) 결과 : " + Arrays.toString(reverse(arr)));

    System.out.println("로그 시간 O(log n) 결과 : "  + binarySearch(arr,15));
  ...
  private static int getFirst(int[] arr) {
    ...
    /* 입력된 배열의 크기가 아무리 커지더라도 연산은 딱 한 번(수행)*/
...
/*     2. 찾으려는 target이 중간보다 크다면 (mid 보다 오른쪽에 있을 경우)*/
      if(arr[mid]>target )
        left = mid + 1;

        //3. 찾으려는 target이 중간보다 작다면(mid 보다 왼쪽에 있읋 경우)
      else{
        right = mid -1;
      }
    }
  ...
  /* 지수 시간 0(2^n)
  * - 입력 크기(n) 이증가 할 때 2n의 n제곱에 비례하여 시간이 기하급수적으로 능가하는 경웅를 의미
  * */

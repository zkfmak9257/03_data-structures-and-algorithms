...
public class MyArrayList<T> {
    ...
    * */
    ...
    // 사용자가 리스트의 용량을 지정하지 않을 경우 사용할 기본값.
    private static final int DEFAULT_CAPACITY = 10;

    /* 기본 생성자 - 초기 용량 10으로 생성
    *  @SuppressWarnings("unchecked")
    *   - 해당 부분에서 발생하는 제네릭 관련 안전선 경고를 무시하라고 컴파일러에게 알려주는 어노테이션
    * */
    @SuppressWarnings("unchecked")
    public MyArrayList() {
        // Object배열 생성 후 T[]로 강제 형변환 -> Java에서는 제네릭 배열을 직접 생성할 수 없기 때문이다
    ...
    /**
     * 지정된 인덱스에 요소를 추가
     * 시간 복잡도: O(n) - 삽입 위치 뒤의 모든 요소를 뒤로 한 칸씩 밀어야 됨.
     * @param index
     * @param element
     * @throws IndexOutOfBoundsException
     */
    public void add(int index, T element) {
        // 1. index가 data배열의 유효한 범위인지 확인
    ...
    /**
     * 지정된 index의 요소를 리스트에서 제거하고 제거된 요소를 반환
     * 시간 복잡도: O(n) - 제거된 요소 뒤 모든 요소를 한 칸씩 당겨야함
     * @param index
     * @return T
     */
    public T remove(int index) {
        ...
        // 3. 제거된 위치 뒤의 요소를 한 칸씩 당기기
        for (int i = index; i < size - 1; i++) {
            data[i] = data[i + 1];
        }

        // 4. 마지막 요소를 null로 변경하고 size 1감소
        data[--size] = null;

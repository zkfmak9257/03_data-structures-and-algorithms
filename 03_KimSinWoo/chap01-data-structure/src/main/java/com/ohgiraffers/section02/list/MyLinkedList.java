...
public class MyLinkedList<T> {
     ...
     * @throws IndexOutOfBoundsException 인덱스 범위가 유효 범위를 벗어났을 때
     */
    public void add(int index, T element) {
        ...
        if (index == 0) {
            ...
            if (size == 0) {
                tail = newNode;     // 기존에 저장된 노드가 없다면 tail도 newNode를 가리키도록 지정
            }
        }
        // 3-2. 삽입 위치가 맨 뒤(tail)인 경우
        else if (index == size) {
            tail.next = newNode;    // 현재 tail이 다음 새 Node 가 되고
        ...
        // 3-3. 삽입 위치가 중간인 경우
            ...
            newNode.next = current.next; // 새 Node의 다음이 current의 다음
     ...
     * 요소 제거 후 headㄷ 또는 tail을 갱신할 가능성이 있다.
     * 시간 복잡도: O(n)
    ...
    /**
     * head부터 tail까지 모든 노드를 차례대로 순회하면서 StringBuilder에 data를 추가한 후

...
/* 이진 트리(Binary Tree)
 * - 각 노드가 최대 두 개의 자식을(L, R) 가지는 트리 구조
 *
 * 이진 탐색 트리(Binary Search Tree)
 * - 이진 트리 구조를 가지면서 정렬 속성을 만족하는 트리
 * - 모든 노드의 왼쪽 서브 트리에 있는 모든 노드의 값은 현재 노드 값보다 작다.
 * - 모든 노드의 오른쪽 서브 트리에 있는 모든 노드의 값은 현재 노드 값보다 크다.
 * - 모든 서브 트리 또한 이진 탐색 트리다.
 *
...
public class BinarySearchTree<T extends Comparable<T>> {
  ...
  private Node<T> insertRec(Node<T> node, T data) {
    ...
    // 현재 데이터가 node.data와 같을 경우 -> 중복이라서 삽입 X
  ...
  private boolean searchRec(Node<T> node, T data) {
    ...
    return data.compareTo(node.data) < 0
        ? searchRec(node.left, data) // 찾을 값이 작으면 왼쪽
        : searchRec(node.right, data); // 찾을 값이 크면 오른쪽
  }
  ...
  private Node<T> deleteRec(Node<T> node, T data) {
    ...
    if (data.compareTo(node.data) < 0) {
      ...
      // 1) 자식 노드가 왼쪽이 없을 때
      if (node.left == null){ return node.right; } // 오른쪽 자식 반환

      // 2) 자식 노드가 오른쪽이 없을 때
      else if (node.right == null){ return node.left; } // 왼쪽 자식 반환

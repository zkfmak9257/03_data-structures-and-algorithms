...
/*
 *  단일 연결 리스트(Singley Linked List) 직접 구현하기
...
public class MyLinkedList<T> {
    ...
    private static class Node<T> {
        ...
        Node(T data) {
            this.data = data;
            this.next = null;   // 처음 노드 생성 시 다음 노드 미지정
        }
    ...
    public void add(int index, T element) {
        ...
        // 3-3. 삽입 위치가 중간인 경우
            ...
            for (int i = 0; i < index - 1; i++) {
                current = current.next;     // index - 1 번째 노트가 최종 current
            }
     ...
     * 요소 제거 후 head또는 tail을 갱신할 가능성이 있다.
     * 시간 복잡도 : 최악의 경우 O(n)
     *
     * @param index 제거 할 요소의 index 번호
     * @return 제거된 요소
     */

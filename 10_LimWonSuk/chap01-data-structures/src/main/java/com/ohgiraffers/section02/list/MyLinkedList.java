 ...
 * 각 node는 data + 다음 node를 가리키는 포인터로 구성되어 있다.
 * */
public class MyLinkedList<T> {   // ★ 제네릭 추가
    ...
    private static class Node<T> {
        ...
        Node(T data) {
            this.data = data; // this = Node
            this.next = null; // 노드 처음 생성시 다음 노드 미지정
        }
    }
    ...
    /**
     * 리스트의 끝에 전달받은 요소를 추가
     * 시간 복잡도 : 0(1) - tail 포인터를 이용해서 바로 추가 가능
     * @param element
     */
    public void add(T element) {
        ...
        if (head == null) {
            ...
            head = newNode;   // ★ 대입 연산자 수정
            tail = newNode;   // ★ 대입 연산자 수정
    ...
    /**
     * 지정된 인덱스에 요소를 삽입
     * 삽입 위차가 리스트의 제일 끝이면 tail 포인터를 갱신
     * 시간 복잡도 : 최악의 경우 O(n)
     * @param index 삽입할 위치
    ...
    public void add(int index, T element){
        ...
        // 3-3. 삽입 위치가 맨 중간인 경우
            ...
            for (int i = 0; i < index - 1; i++) {
                ...
                // 사이에 들어가서 참조하던걸 하나씩 밀거나 당겨서 서로바라보는 index 사이에 들어간다
                // index - 1 번째 노드까지 이동 -> 1번째 Node 가 최종 current가됨
            }
    ...
    /**
     * "Node를 계속이동하면서 StringBuilder에 추가하는것"

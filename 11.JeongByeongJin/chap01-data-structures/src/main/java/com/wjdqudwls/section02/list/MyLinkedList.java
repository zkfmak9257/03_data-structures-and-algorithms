 ...
 * 각 노드는 데이터 + 다음 노드를 가리키는 포인터
 * */

public class MyLinkedList<T> {

  private Node<T> head; // 리스트의 첫 번째 노드를 가르키는 포인터
  private Node<T> tail; // 리스트의 마지막 노드를 가르키는 포인터
  private int size;
  ...
  private static class Node<T> {
    T data;       // 노드를 저장할 실제 데이터
    Node<T> next; // 다음 노드를 가르키는 포인터
  ...
  public void add(int index, T element) {
    ...
    if (index == size) {
      newNode.next = head; // 새 노드의 다음 여소를 현재 head로 지정
      head = newNode; // head 포인터가 새 노드를 가리키도록 지정
      if (size == 0) { // 기존 저장된 요소가 없다면 head, tail 모두 newNode를 가르켜야함
        tail = newNode;
      }
    }
   ...
   * 오소 제거 후 head 또는 tail을 갱신할 가능성이 있다.
   * 시간 복잡도 : 최악의 경우 O(n)
   *
  ...
  /**
   * head 부터 tail까지 모든 Node를 차례대로 순회 하면서

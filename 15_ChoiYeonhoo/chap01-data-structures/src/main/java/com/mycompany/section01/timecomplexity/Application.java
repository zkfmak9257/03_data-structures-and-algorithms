package com.mycompany.section01.timecomplexity;
...
public class Application {
  public static void main(String[] args) {
    ...
    System.out.println("선형 시간 O(n)의 결과 : " + Arrays.toString(reverse(arr)));

    System.out.println("로그 시간 O(log n)의 결과 인덱스 : " + binarySearch(arr,15));

    System.out.println("지수 시간 O(2^n)의 결과 : " + fibonacci(10));
  ...
  * */
  ...
  /* 로그 시간 O(log n)
  * - 입력의 크기(n)가 증가할 때, 실행 시간이 로그 함수적으로 증가하는 경우를 의미
  ...
  * */
  ...
  private static int binarySearch(int[] arr, int target){
    ...
    */
    /* Arrays.sort(arr)
    * - 배열 원본 정렬
    * - Array.sort()는 일반적으로 O(n log n)의 시간 복잡도를 가짐.
    *  -> 지금은 정렬 시간은 제외하고 생각하기 */
    Arrays.sort(arr);
...
//      int mid = (left + right) / 2;
    ...
    // 찾으려는 값이 배열내에 없거나, 정렬 안됨
    return -1;
  }

  /* 지수 시간 O(2^n)
  * - 입력 크기(n) 증가할 때 마다 2의 n제곱에 비례하여

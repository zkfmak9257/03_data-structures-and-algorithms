...
public class MyLinkedList<T> {
  ...
  * */
  ...
  /**
   * 리스트의 끝에 전달 받은 요소를 추가
   * 시간 복잡도 : O(1) - tail 포인터를 이용해서 바로 추가가 가능
   * @param element
   */
  public void add(T element){
    // 1. 새 Node 생성 + Node에 데이터 저장
    ...
    if (head == null) {
      ...
      // 3. 리스트 내 저장된 요소가 있다면
      tail.next = newNode; // 기존 꼬리의 다음 참조 주소를 새로 만든 요소에 연결하고
      tail = newNode; // 꼬리 위치를 새로 만든 요소로 옮김
    }
    // 4. 리스트에 저장된 요소 개수 size를 1증가
    size++;
  }
  ...
  public void add(int index, T element){
    ...
    if (index < 0 || index > size){
      throw new IndexOutOfBoundsException("인덱스가 범위를 벗어 났습니다");
    }
    ...
    if(index == 0){
      newNode.next = head; // 새 노드의 다음 요소를 현재 head로 지정, 기존 데이터가 없었다면 head 값이 null이라서 상관 없음
      head = newNode; // head pointer가 새 노드를 가리키도록 지정
      
      if (size == 0){ // 기존 저장된 요소가 없다면 
        tail = newNode;
      }
    }
    ...
    // 3-3. 삽입 위치가 중간인 경우
      ...
      // 삽입할 이전 위치의 노드 찾기
      Node<T> current = head; // 헤드 부터 하나씩 접근
      for (int i = 0; i < index-1; i++) {
        current = current.next; // (index-1)번째 노드가 최종 current
        }

      newNode.next = current.next; // 우선 새로 만들어진 Node의 next 값으로 (index-1)번째 노드(current)가 가지고 있던 다음 node 주소를 대입하고
      current.next = newNode; // current의 다음 주소가 newNode가 되도록 대입한다.
      }
  ...
  /**
   * head부터 tail 까지 모든 Node를 차례대로 순회하면서

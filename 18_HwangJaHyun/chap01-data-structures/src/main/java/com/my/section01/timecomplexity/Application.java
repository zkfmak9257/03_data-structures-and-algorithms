package com.my.section01.timecomplexity;
...
* 특정알고리즘이 대규모 데이터 셋에서 얼마나 잘 동작할지 예측하는데 중요함.
* */
public class Application {
  public static void main(String[] args) {
    ...
    System.out.println("로그 시간 O(log n) 결과 인덱스 : " + binarySearch(arr,15));

    System.out.println("지수 시간 0(2^n) 결과 : " + fibonacci(6));
  }
  ...
  * 흔한 예시 : 배열 / 리스트 순회
  * 이점 : 입력 크기에 따라 예측가능한 성능을 제공한다.
  * */
  private static int[] reverse1(int[] arr){
    // 전달받은 arr의 요소 순서를 뒤집어서 반환
...
//        .toArray();
  ...
  /* 로그 시간 O(log n)
  * 입력 크기(n)이 증가할 때, 실행 시간이 로그 함수적으로 증가하는 경우를 의미
  ...
  * */
  ...
  /* 지수 시간 O(2^n)
  * 입력 크기(n)이 증가할 때마다 2의 n 제곱에 비례하여
